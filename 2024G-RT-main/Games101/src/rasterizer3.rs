        ...
        for x in min_x..=max_x {
            for y in min_y..=max_y {
                ...
                if z_interpolated < self.depth_buf[Rasterizer::get_index(self.width, self.height, x, y)] {
                    ...
                    let pixel_color = match self.fragment_shader { None => Vector3::zeros(), Some(f) => f(&payload), };
                    Rasterizer::set_pixel(self.width, self.height, &mut self.frame_buf, &Vector3::new(x as f64, y as f64, 1.0), &pixel_color);
                }
            }
        }
